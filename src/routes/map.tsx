...
function LiveMap() {
  ...
  const bagsOnMap = bags
    ...
    .map((b) => {
      const zoneName =
        b.lastSeenZone ??
        "TAGGING_STATION".replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      const loc = MAP_LOCATIONS.find((l) => l.name.toLowerCase() === zoneName.toLowerCase());
      return {
        tag: b.id,
        flight: b.flightNo,
        passenger: "—",
        x: loc?.x ?? 50,
        y: loc?.y ?? 50,
        status: b.status === "ALARMED" ? "ALARM" : "TRACKING",
      };
    });

  const bag = bagsOnMap.find((b) => b.tag === selected);

  const fullBag = bags.find((b) => b.id === selected);
  const bagEvents = fullBag
    ? useAppStore
        .getState()
        .events.filter((e) => e.epc === fullBag.epc)
  ...
  const dwellMinutes =
    firstEvent && lastEvent
      ? Math.round(
          (new Date(lastEvent.lastSeen).getTime() - new Date(firstEvent.firstSeen).getTime()) /
            60000,
        )
      : 0;
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2 text-[12px]">
            ...
            <button className="px-2.5 py-1.5 border border-border rounded-md hover:bg-accent inline-flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-12 gap-4">
          ...
          <div className="relative w-full h-160 bg-white scan-grid">
            ...
            >
              ...
              <rect
                ...
                strokeWidth="0.15"
              />
              <text x="10" y="22" fontSize="2" fill="rgb(71,85,105)">
                RECLAIM HALL
              </text>
              {/* Belts */}
              {[14, 28, 42].map((x, i) => (
                <g key={i}>
                  <rect
                    ...
                    height="8"
                    rx="1"
                    fill="oklch(0.94 0.02 220)"
                    stroke="oklch(0.80 0.03 220)"
                    strokeWidth="0.15"
                  />
                  <text x={x + 1.5} y="31.5" fontSize="1.6" fill="rgb(100,116,139)">
                    Belt {i + 1}
                  </text>
                </g>
              ))}
              ...
              <rect
                ...
                strokeWidth="0.15"
              />
              <text x="42" y="46" fontSize="2" fill="rgb(71,85,105)">
                ARRIVAL HALL
              </text>
              {/* Customs corridor */}
              <rect
                ...
                stroke="oklch(0.75 0.10 30)"
                strokeWidth="0.2"
              />
              <text x="42" y="78" fontSize="2" fill="rgb(220,140,120)">
              ...
              <rect
                ...
                strokeWidth="0.15"
              />
              <text x="21" y="58" fontSize="1.6" fill="rgb(71,85,105)">
                WC N
              </text>
              <rect
                x="64"
                ...
                strokeWidth="0.15"
              />
              <text x="65" y="68" fontSize="1.6" fill="rgb(71,85,105)">
                WC S
              </text>
              {/* Lost & found */}
              <rect
                ...
                strokeWidth="0.15"
              />
              <text x="73" y="26" fontSize="1.6" fill="rgb(71,85,105)">
                L&amp;F
              </text>
              ...
              {bagsOnMap.map((b, i) => (
                <line
                  ...
                  x2={b.x}
                  y2={b.y}
                  stroke={b.status === "ALARM" ? "oklch(0.65 0.25 25)" : "oklch(0.72 0.15 230)"}
                  strokeWidth="0.25"
            ...
            {MAP_LOCATIONS.map((l) => {
              const color =
                l.kind === "alarm" ? "bg-danger" : l.kind === "reader" ? "bg-info" : "bg-success";
              return (
                <div
                  key={l.id}
                ...
                >
                  <div
                    className={`size-3 rounded-full ${color} ring-2 ring-background ${l.kind === "alarm" ? "glow-danger" : ""}`}
                  />
            ...
            {bagsOnMap.map((b) => (
              ...
              >
                <div
                  className={`size-5 rounded-md border-2 ${b.status === "ALARM" ? "bg-danger/30 border-danger glow-danger" : "bg-info/30 border-info"} flex items-center justify-center`}
                >
            ...
            <div className="absolute right-3 top-3 flex flex-col rounded-md border border-border bg-panel/90 backdrop-blur">
              <button className="size-8 hover:bg-accent flex items-center justify-center border-b border-border">
                <Plus className="size-4" />
              </button>
              <button className="size-8 hover:bg-accent flex items-center justify-center">
            ...
            <div className="absolute left-3 bottom-3 bg-panel/90 backdrop-blur border border-border rounded-md p-3 text-[11px] space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <span className="size-2.5 rounded-full bg-info" /> RFID Reader
              </div>
              <div className="flex items-center gap-2">
                <span className="size-2.5 rounded-full bg-success" /> Normal Area
              </div>
              <div className="flex items-center gap-2">
                <span className="size-2.5 rounded-full bg-danger animate-pulse" /> Active Alarm
              </div>
              <div className="flex items-center gap-2">
                <Briefcase className="size-3 text-foreground" /> Suspect Bag
              </div>
            </div>
            {/* Floor indicator */}
        ...
        <div className="col-span-12 xl:col-span-3 space-y-4">
          {bag ? (
            ...
            >
              <div className="space-y-2.5">
                ...
                <dl className="grid grid-cols-3 gap-y-1.5 text-[12px]">
                  <dt className="col-span-1 text-muted-foreground">Flight</dt>
                  <dd className="col-span-2 font-mono">{bag.flight}</dd>
                  <dt className="col-span-1 text-muted-foreground">Passenger</dt>
                  <dd className="col-span-2">{bag.passenger}</dd>
                  <dt className="col-span-1 text-muted-foreground">Location</dt>
                  <dd className="col-span-2">{fullBag?.lastSeenZone?.replace(/_/g, " ") ?? "—"}</dd>
                  <dt className="col-span-1 text-muted-foreground">First seen</dt>
                  <dd className="col-span-2 font-mono">
                    {firstEvent
                      ? new Date(firstEvent.firstSeen).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                          second: "2-digit",
                        })
                  ...
                  <dt className="col-span-1 text-muted-foreground">Dwell time</dt>
                  <dd className={`col-span-2 font-mono ${dwellMinutes > 30 ? "text-warning" : ""}`}>
                    {dwellMinutes} min
                  </dd>
                </dl>
                <div className="flex gap-2 pt-1">
                  <a
                    href="/target"
                    className="flex-1 text-center text-[12px] px-2.5 py-1.5 rounded-md border border-border hover:bg-accent"
                  >
                    Open profile
                  </a>
                  <button className="flex-1 text-[12px] px-2.5 py-1.5 rounded-md bg-danger text-destructive-foreground font-medium">
                    Escalate
                  </button>
                </div>
              </div>
            ...
            <Panel title="Suspect Bag Details">
              <div className="text-[12px] text-muted-foreground py-6 text-center">
                Select a bag on the map to inspect.
              </div>
            </Panel>
          )}
                  ...
                  <AlertTriangle
                    className={`size-3 mt-0.5 ${
                      e.eventType.includes("ALARM") || e.eventType.includes("EXIT")
                        ? "text-danger"
                        : e.eventType.includes("RESTRICTED") || e.eventType.includes("ESCAPE")
                          ? "text-warning"
                          : "text-info"
                    }`}
                  />
