...
function Target() {
  ...
  const bag = paramBagId
    ...
    : bags.find((b) => b.status === "ALARMED" || b.status === "AT_RECHECK");

  const bagAlarm = bag
    ? alarms.find(
        (a) => a.bagId === bag.id && a.outcome !== "CLEARED" && a.outcome !== "SUPPRESSED",
      )
    : null;

  const bagEvents = bag?.epc
    ? events
        .filter((e) => e.epc === bag.epc)
    ...
    return (
      <div className="p-6">
        ...
        <div className="py-12 text-center text-[13px] text-muted-foreground">
          Select a bag from the Alarms page (click the View icon) or use the Simulator to create
          one.
        </div>
      </div>
    );
  ...
  const FIELDS = [
    ["Tag ID", bag.id],
    ["BHS UID", bag.bhsUid ?? "Not supplied"],
    ["Flight", bag.flightNo],
    ["EPC", bag.epc || "Not assigned"],
    ["Classification", "Suspect Bag"],
    ["Status", bag.status.replace(/_/g, " ")],
    ["Current Location", bag.lastSeenZone?.replace(/_/g, " ") ?? "Not seen"],
    [
      "First Seen",
      firstEvent
    ...
    <div className="p-6">
      <div className="print-only text-center mb-4 pb-3 border-b">
        ...
        <div className="text-[11px] text-muted-foreground">
          {bag.id} · {bag.flightNo} · Printed {new Date().toLocaleString()}
        </div>
      </div>
      <PageHeader
        title="Target Information"
        subtitle={`Profile of suspect bag · ${bag.id}`}
        actions={
      ...
      <div className="grid grid-cols-12 gap-4">
        <Panel title="Bag Information" className="col-span-12 lg:col-span-5">
          <div className="flex items-center gap-3 pb-3 border-b border-border mb-3">
            ...
            <div>
              <div className="font-mono text-lg font-semibold">{bag.id}</div>
              <div className="text-[12px] text-muted-foreground">
                {bag.flightNo} · {bag.lastSeenZone?.replace(/_/g, " ") ?? "Not seen"}
              </div>
              <div className="mt-1">
                <StatusPill
                  status={
          ...
          <Panel
            ...
            action={
              <span className="text-[11px] text-muted-foreground">
                {bagNotes.length} note{bagNotes.length !== 1 ? "s" : ""}
              </span>
            }
          >
            {bagNotes.length > 0 && (
