  ...
  const reportCards = [
    {
      ...
      stats: [
        { label: "Bags tagged", value: bags.filter((b) => b.status !== "IDENTIFIED").length },
        {
          ...
          value: bags.filter((b) => !["RESOLVED", "IDENTIFIED"].includes(b.status)).length,
        },
        ...
        {
          label: "Bags held / seized",
          value: resolutions.filter(
            (r) => r.action === "NOT_CLEARED" || r.action === "PROHIBITED_ITEM_SEIZED",
          ).length,
        },
        {
          label: "Duty collected",
          value: resolutions.filter((r) => r.action === "DUTY_COLLECTED").length,
        },
    ...
    },
    {
      ...
      stats: [
        ...
        { label: "Open", value: alarms.filter((a) => a.outcome === "OPEN").length },
        {
          ...
          value: alarms.filter((a) => a.outcome === "UNDER_INVESTIGATION").length,
        },
    ...
    },
    {
      ...
      stats: [
        ...
        {
          label: "Avg read rate",
          value: `${Math.round(readers.reduce((s, r) => s + r.readRate, 0) / readers.length)}%`,
        },
    ...
    },
    {
      ...
      stats: (() => {
        ...
        bags.forEach((b) => {
          if (!flightCounts[b.flightNo]) flightCounts[b.flightNo] = { total: 0, resolved: 0 };
          flightCounts[b.flightNo].total++;
          if (b.status === "RESOLVED") flightCounts[b.flightNo].resolved++;
        });
                ...
                onClick={() => {
                  const bagSheet =
                    ...
                    bags
                      .map(
                        (b) =>
                          `${b.id}\t${b.flightNo}\t${b.epc ?? ""}\t${b.status}\t${b.lastSeenZone ?? ""}`,
                      )
                      .join("\n");
                  ...
                  const blob = new Blob([bagSheet + alarmSheet + eventSheet], {
              ...
              <button
                onClick={() => {
                  ...
                  const rows = bags
                    .map(
                      (b) =>
                        `${b.id},${b.flightNo},${b.epc ?? ""},${b.status},${b.lastSeenZone ?? ""},${b.flaggedAt}`,
                    )
                    .join("\n");
                  ...
                  const blob = new Blob(
                    [header + rows + alarmHeader + alarmRows + eventHeader + eventRows],
                    { type: "text/csv" },
                  );
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
                  ...
                  URL.revokeObjectURL(url);
                  toast.success(
                    `Exported ${bags.length} bags, ${alarms.length} alarms, ${events.length} events`,
                  );
                }}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border text-[12px] hover:bg-accent"
              >
              ...
              >
                ...
                {report.stats.length === 0 && (
                  <div className="py-4 text-center text-[12px] text-muted-foreground">
                    No data — run some scenarios in the Simulator first
                  </div>
                )}
              </Panel>
            );
          ...
          <Panel title="Alarm Heatmap · This Week" className="col-span-12 xl:col-span-7">
            ...
            <div className="mt-3 flex items-center gap-2 text-[10px] text-muted-foreground">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to right, var(--color-secondary), var(--color-danger))",
                }}
              />
              <span>High</span>
            </div>
              ...
              {readerRank.map((r, i) => (
                <li key={r.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-secondary overflow-hidden">
                    <div
                      className={`h-full ${r.v > 90 ? "bg-success" : r.v > 60 ? "bg-warning" : "bg-danger"}`}
                      style={{ width: `${r.v}%` }}
                    />
                  </div>
